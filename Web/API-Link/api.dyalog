...
⍝∇:require =..\..\..\APP\Files.dyalog
    ...
    ∇ r←mode InitAPI(APIHomeDir API APIClassName APIType APIToken sessionId trapping);loadRes;server;port;parms;wsid;runtime
    r←0
      ⎕←'*** Initialising API NOW'
      :If mode≡'inSitu'   ⍝############################ inSitu #############################################⍝
          #.API.⎕CY #.Boot.AppRoot,'API-Link/API-Link'
      ...
      :Else                ⍝############################ Process #############################################⍝
              ...
              parms,←#.HtmlUtils.enlist(⊂' -API'),¨(''  'ClassName' 'HomeDir' 'Type',¨'=',¨(API  APIClassName APIHomeDir APIType))
